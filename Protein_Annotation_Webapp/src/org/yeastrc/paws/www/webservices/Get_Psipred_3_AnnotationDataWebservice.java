...
import org.yeastrc.paws.www.service.Get_Psipred_3_DataService;


@Path( RestWebServicePathsConstants.GET_PSIPRED_3 )

public class Get_Psipred_3_AnnotationDataWebservice {

	Logger log = Logger.getLogger(Get_Psipred_3_AnnotationDataWebservice.class);
...
//		String callbackQueryParamValue = request.getParameter( RestWebServiceJSONP_Constants.CALLBACK_QUERY_PARAMETER );
	...
	public String processGetInternal( String sequenceIdString,
		...
		if ( StringUtils.isEmpty( sequenceIdString ) ) {
			...
			String msg = "'" + RestWebServicePathsConstants.GET_PSIPRED_3
							+ "' query param is not provided or is empty.  URL must be '"
							+ RestWebServicePathsConstants.GET_PSIPRED_3
		...
		try {
			...
			String response = Get_Psipred_3_DataService.getInstance().get_Psipred_3_DataForSequenceId( sequenceId, ncbiTaxonomyId );
