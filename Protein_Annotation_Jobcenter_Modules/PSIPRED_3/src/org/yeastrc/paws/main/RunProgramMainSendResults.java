...
import org.yeastrc.paws.program_runner.PsipredResultsParser;
...
//		restartImageNumber = 0;
	...
	public void processRequest( int trackingId, 
		...
		try {
			...
			PsipredParsedResults parsedResults = PsipredResultsParser.parseResults( tempDirectory );
			...
			String parsedResultsString = GetJSONForObject.getJSONForObject( parsedResults );
			...
			SendResultsToServer.send( trackingId, 
					parsedResultsString /* annotationData */, 
			...
			if ( ! forceSpecifyTempDirNameSpecified ) {
				//  Only remove temp dir if "Force..." is not specified
	...
	private File createInputFile(String sequence, File tempDirectory) throws Throwable, IOException {
		...
		File programInputFile = new File( tempDirectory, PsipredFilenameConstants.PSIPRED_INPUT_FILENAME );
